...
SvLocal void QBStandbyLogicEnterPassiveStandby_(QBStandbyLogic self)
{
    ...
    QBPlatformSetLEDBrightness(led, QBPlatformLEDBrightness_max); // set front panel LED to red for homecast/hnc platforms (off - blue, max - red)
    for (led = 1; led < count; led++) {
        QBPlatformSetLEDBrightness(led, QBPlatformLEDBrightness_off); // disable all other LEDs (Intek hasn't any, but count is 2)
    }
}
