...
// BEGIN VODAdultPinAuth
SvLocal void QBInnov8onCarouselMenuCheckPinCallback(void *self_, SvWidget dlg, SvString ret, unsigned key)
{
    QBInnov8onCarouselMenuChoice self = self_;
...
SvLocal void QBInnov8onCarouselMenuCheckPin(QBInnov8onCarouselMenuChoice self)
{
    svSettingsPushComponent("Carousel_VOD.settings");
    SvObject authenticator = QBAuthenticateViaAccessManager(self->appGlobals->scheduler, self->appGlobals->accessMgr, SVSTRING("PC_VOD"));
    SvWidget master = NULL;
    svSettingsPushComponent("ParentalControl.settings");
    SvWidget dialog = QBAuthDialogCreate(self->appGlobals, authenticator, gettext("Authentication required"), gettext("Please enter parental control PIN to enter this service"), true, NULL, &master);
    svSettingsPopComponent();
    self->parentalPopup = dialog;
    QBDialogRun(dialog, self, QBInnov8onCarouselMenuCheckPinCallback);
}
// END VODAdultPinAuth
...
QBCarouselMenuNodeSelected(SvObject self_, SvObject node_, SvObject nodePath)
{
    ...
    if (SvObjectIsInstanceOf(node_, QBContentSearch_getType())) {
        ...
        // BEGIN VODAdultPinAuth
        const char *VOD_ADULT_PIN_AUTH = QBConfigGet("VOD_ADULT_PIN_AUTH");
        bool isAdultPINAuth = VOD_ADULT_PIN_AUTH && (strcmp(VOD_ADULT_PIN_AUTH, "on") == 0);
        
        bool isAdultPINAuthAndAdultService = isAdultPINAuth && (SvStringEqualToCString(self->serviceName,"Adultos") || 
                                SvStringEqualToCString(self->serviceName,"HotGo") || SvStringEqualToCString(self->serviceName,"Divas TV") ||
                                SvStringEqualToCString(self->serviceName,"DivasTV") || SvStringEqualToCString(self->serviceId, "53") || 
                                SvStringEqualToCString(self->serviceId, "54") || SvStringEqualToCString(self->serviceId, "55"));
        if (isAdultPINAuthAndAdultService)
        {
            SVTESTRELEASE(self->contentSearchPath);
            self->contentSearchPath = SvObjectCopy(nodePath, NULL);
            QBInnov8onCarouselMenuCheckPin(self);
            return true;
        }
        else
        {
