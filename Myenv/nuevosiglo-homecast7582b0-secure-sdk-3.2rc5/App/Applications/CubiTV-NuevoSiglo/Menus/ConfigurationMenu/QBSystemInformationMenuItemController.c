                                                                ...
                                                                QBXMBItem item)
{
    ...
    if (networkWatcher && !self->isNetworkAvailable) {
        item->subcaption = SvStringCreate(gettext("Local connection error"), NULL);
        return;
    }
    ...
    switch (self->customerInfoMontitorStatus) {
        case CubiwareMWCustomerInfoStatus_ok:
            ...
            item->subcaption = SvStringCreate(gettext("Connection established"), NULL);  // AMERELES Add description in middleware status
            break;
        case CubiwareMWCustomerInfoStatus_error:
            ...
            // BEGIN AMERELES Add description in middleware status
                const char *errorDesc;
                switch (errorCode) {
                    case 0xf000: errorDesc = gettext("No error (request did not failed)");break;
                    case 0xf001: errorDesc = gettext("Internal library error");break;
                    case 0xf002: errorDesc = gettext("Invalid behavior of HTTP client listener");break;
                    case 0xf003: errorDesc = gettext("Could not resolve host name");break;
                    case 0xf004: errorDesc = gettext("Could not connect to host or proxy");break;
                    case 0xf005: errorDesc = gettext("Download haven't been completed");break;
                    case 0xf006: errorDesc = gettext("HTTP server returned error");break;
                    case 0xf007: errorDesc = gettext("Operation timed out");break;
                    case 0xf008: errorDesc = gettext("I/O error while sending data");break;
                    case 0xf009: errorDesc = gettext("I/O error while receiving data");break;
                    case 0xf00a: errorDesc = gettext("Remote resource not found");break;
                    case 0xf00b: errorDesc = gettext("Error in SSL/TLS handshake");break;
                    case 0xf00c: errorDesc = gettext("Local client certificate could not be loaded");break;
                    case 0xf00d: errorDesc = gettext("Local CA certificates could not be loaded");break;
                    case 0xf00e: errorDesc = gettext("Could not use SSL/TLS cipher");break;
                    case 0xf00f: errorDesc = gettext("Peer certificate could not be verified");break;
                    case 0xf010: errorDesc = gettext("Other unspecified error");break;
                    case 0x3:    errorDesc = gettext("The card is not whitelisted");break;
                    case 0x47:   errorDesc = gettext("Device is not assigned to any customer");break;
                    case 0x1f:   errorDesc = gettext("Unregistered card");break;
                    default:     errorDesc = gettext("Unknown error");break;
                }
                item->subcaption = SvStringCreateWithFormat("%s (%s #%04X)", errorDesc, gettext("Error"), errorCode);
                // END AMERELES Add description in middleware status
            break;
