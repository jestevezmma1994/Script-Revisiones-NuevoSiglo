...
#include <Widgets/AnimationCountdownDialog.h>
...
SvLocal void QBConfigurationMenuUpdateVODServicesConfirmationCallback(void *self_, SvWidget dialog, SvString buttonTag, unsigned keyCode)
{
    QBConfigurationMenuRestrictedNodesHandler self = self_;
    self->dialog = NULL;
}

SvLocal void QBConfigurationMenuUpdateVODServicesDialogCallback(void *self_, SvWidget dialog, SvString buttonTag, unsigned keyCode)
{
    SvLogNotice("%s() UpdateVODServices feature, end animation dialog", __func__);
    QBConfigurationMenuRestrictedNodesHandler self = self_;
    self->dialog = NULL;

    char* confirmationTextDesc = NULL;
    if (QBProvidersControllerServiceGetUpdateVODServicesCount(self->appGlobals->providersController)) {
        SvLogNotice("%s() UpdateVODServices finish successfully", __func__);
        confirmationTextDesc = gettext("VOD services updated");
    } else {
        SvLogNotice("%s() UpdateVODServices timeout without response", __func__);
        confirmationTextDesc = gettext("VOD services not updated");
    }

    QBConfirmationDialogParams_t params = {
        .title = confirmationTextDesc,
        .message = "",
        .local = true,
        .focusOK = true,
    ...
    self->dialog = QBConfirmationDialogCreate(self->appGlobals->res, &params);
    QBDialogRun(self->dialog, self, QBConfigurationMenuUpdateVODServicesConfirmationCallback);
}
...
void QBConfigurationMenuShowUpdateVODServicesDialog(QBConfigurationMenuRestrictedNodesHandler self)
{
    ...
    self->dialog = AnimationCountdownDialogCreate(self->appGlobals);
    QBDialogRun(self->dialog, self, QBConfigurationMenuUpdateVODServicesDialogCallback);
    svSettingsPopComponent();
}
