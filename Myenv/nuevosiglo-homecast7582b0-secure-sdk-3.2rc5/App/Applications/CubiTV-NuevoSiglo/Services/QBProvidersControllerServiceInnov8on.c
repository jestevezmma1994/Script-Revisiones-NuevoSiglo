...
struct QBProvidersControllerServiceInnov8on_t {
    ...
    int UpdateVODServicesCount; // AMERELES Update VoD Services
};
...
QBProvidersControllerServiceInnov8onMonitorServiceAdded(QBProvidersControllerServiceInnov8on self, SvDBObject service, SvString serviceID)
{
    ...
    if (serviceType && SvStringEqualToCString(serviceType, "Social media")) {
        ...
        if (!vodFactory) {
            ...
            // BEGIN AMERELES [#2094] PROXY de Presentación
            if (BoldIsMWProxyService(serviceID))
            {
                externalUrl = SvStringCreate(QBConfigGet("mwPROXY.URL"), NULL);
                call2Action = true;
                externalID = SvStringCreate(SvStringCString(serviceID), NULL);
            }
            // END AMERELES [#2094] PROXY de Presentación
            carouselMenu =
...
QBProvidersControllerServiceInnov8onMonitorStatusChanged(SvObject self_, SvDBObject service, QBMWServiceMonitorEvent event)
{
    ...
    int serviceCountOld = QBProvidersControllerServiceInnov8onGetUpdateVODServicesCount((SvObject)self);
    QBProvidersControllerServiceInnov8onSetUpdateVODServicesCount((SvObject)self, serviceCountOld + 1);
...
// BEGIN AMERELES Update VoD Services
void QBProvidersControllerServiceInnov8onUpdateVODServices(SvObject self_)
{
    QBProvidersControllerServiceInnov8on self = (QBProvidersControllerServiceInnov8on) self_;
    ...
    if (status) {
        SvLogNotice("%s():%i smart card status: %s", __FILE__,__LINE__,status);
        free(status);
    }
    if ( cardStatus == QBCASSmartcardState_inserted || cardStatus == QBCASSmartcardState_correct )
        QBMWServiceMonitorSendRequest(self->serviceMonitor);
    else
        SvLogNotice("%s():%i ERROR: smart card is not inserted", __FILE__,__LINE__);
}

int QBProvidersControllerServiceInnov8onGetUpdateVODServicesCount(SvObject self_)
{
    QBProvidersControllerServiceInnov8on self = (QBProvidersControllerServiceInnov8on) self_;
    return self->UpdateVODServicesCount;
}

void QBProvidersControllerServiceInnov8onSetUpdateVODServicesCount(SvObject self_, int val)
{
    QBProvidersControllerServiceInnov8on self = (QBProvidersControllerServiceInnov8on) self_;
    self->UpdateVODServicesCount = val;
}
// END AMERELES Update VoD Services

// AMERELES #2206 Canal lineal que lleve a VOD
SvObject
QBProvidersControllerServiceInnov8onGetInnov8onServiceByID(SvObject self_, SvString tag, SvString serviceID)
{
    QBProvidersControllerServiceInnov8on self = (QBProvidersControllerServiceInnov8on) self_;
    
    SvArray mounts = (SvArray) SvHashTableFind(self->innov8onMounts, (SvObject) tag);
    if (!mounts || SvArrayCount(mounts) == 0)
        return NULL;
    
    size_t count = SvArrayCount(mounts);
    if (serviceID)
    {
        QBCarouselMenuItemService aux = NULL;
        for (size_t idx = 0; idx < count; idx++)
        {
            aux = (QBCarouselMenuItemService) SvArrayAt(mounts, idx);
            if (aux && SvStringEqualToCString(QBCarouselMenuGetServiceID(aux), SvStringCString(serviceID)))
            {
                return (SvObject) aux;
            }
        }
