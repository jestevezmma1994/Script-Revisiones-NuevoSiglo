    ...
    if(!self->dialog.widget) {
        ...
        while((next = (QBReminderTask) SvIteratorGetNext(&it))) {
            if(next->startTime <= now + self->remindTimeOffset) {
                ...
                // BEGIN AMERELES [4266] Función Recordarme no funciona
                else if (next->endTime > now && SvHashTableFind(self->acceptedTasks, (SvGenericObject) next))
                {
                    QBReminderRemind(self, next);
                    SvHashTableRemove(self->acceptedTasks, (SvObject) next);
                    QBReminderRemoveTask(self, next);
                    break;
                }
                // END AMERELES [4266] Función Recordarme no funciona
            } else
                break;
        }
