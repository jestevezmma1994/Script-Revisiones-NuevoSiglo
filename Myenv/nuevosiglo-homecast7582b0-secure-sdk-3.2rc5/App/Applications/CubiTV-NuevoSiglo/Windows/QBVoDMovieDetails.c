...
struct QBVoDMovieDetailsContext_t {
    ...
    bool VOD3FakeCategoriesApply;
    SvString categoryName; // AMERELES #2131 tracking de navegación VOD en STBs híbridos
    SvString recomendedFrom; // AMERELES #2131 tracking de navegación VOD en STBs híbridos
};
...
struct QBVoDMovieDetails_t {
    ...
    SvWidget logoIcon; // AMERELES Add logo NS
                                                                     ...
                                                                     SvString ret, unsigned key)
{
    ...
    if (ret && SvStringEqualToCString(ret, "OK-button")) {
        ...
        SvValue titleV = (SvValue) SvDBRawObjectGetAttrValue(self->ctx->object, "title");
        SvString title = NULL;
        if (!titleV)
            titleV = (SvValue) SvDBRawObjectGetAttrValue(self->ctx->object, "name");
        if (titleV)
            title = SvValueGetString(titleV);
        else
            title = SVSTRING("");
        
        QBWindowContext movieDetailsCtx = QBVoDMovieDetailsContextCreate(ctx->appGlobals, (QBContentCategory) category, self->similarCarouselActiveElement, ctx->provider, ctx->serviceId, ctx->serviceName, (SvObject) ctx->tree, ctx->categoryName, SvStringEqualToCString(ctx->recomendedFrom, "") ? title : ctx->recomendedFrom);
                                                             ...
                                                             SvString ret, unsigned key)
{
    ...
    SvGenericObject contentInfo = QBVoDMovieDetailsContextCreateContentInfo((QBWindowContext) self->ctx);
    // AMERELES #2131 tracking de navegación VOD en STBs híbridos
    QBContentSideMenuSetContentInfo(contentSideMenu, contentInfo, self->ctx->categoryName, self->ctx->recomendedFrom);
...
QBVoDMovieDetailsCheckParentalControl(QBVoDMovieDetails self, SvDBRawObject movie)
{
    ...
    if (!parentalControl || QBParentalControlRatingAllowed(self->ctx->appGlobals->pc, parentalControl) || self->ctx->VOD3FakeCategoriesApply) //AYOUNG Ignore PC in VOD3FakeCategories
        allowed = true;
    return allowed;
}
...
SvLocal void QBVoDMovieDetailsCoverFocusEventHandler(SvWidget w, SvFocusEvent e)
{
    ...
    switch (e->kind) {
        case SvFocusEventKind_GET:
            // { AYOUNG VOD3FakeCategories
            if (self->movieDetails->ctx->VOD3FakeCategoriesApply)
            {
                SvLogNotice("%s VOD3FakeCategories feature detected, the event to focus the cover will be ignored", __func__);
                return;
            }
...
SvLocal bool QBVoDMovieDetailsCoverInputEventHandler(SvWidget w, SvInputEvent e)
{
    ...
    QBVoDMovieDetailsCover self = w->prv;
    // { AYOUNG VOD3FakeCategories
    if (self->movieDetails->ctx->VOD3FakeCategoriesApply && e->ch == QBKEY_ENTER)
    {
        SvLogNotice("%s VOD3FakeCategories feature detected, enter key in cover will be ignored", __func__);
        return true; // TODO ver si es true o false
    }
    // } AYOUNG VOD3FakeCategories
    if (e->ch == QBKEY_ENTER) {
            ...
            if (notify->type == QB_CAROUSEL_NOTIFICATION_ACTIVE_ELEMENT) {
                ...
                if (notify->dataObject && SvObjectIsInstanceOf(notify->dataObject, SvDBRawObject_getType())) {
                    ...
                    if(QBVoDMovieDetailsCheckParentalControl(self->movieDetails, obj)) {
                        SvObject category = SvInvokeInterface(QBTreeModel, ctx->tree, getNode, NULL);
                        // AMERELES #2131 tracking de navegación VOD en STBs híbridos
                        SvValue titleV = (SvValue) SvDBRawObjectGetAttrValue(currentMovie, "title");
                        SvString title = NULL;
                        if (!titleV)
                            titleV = (SvValue) SvDBRawObjectGetAttrValue(currentMovie, "name");
                        if (titleV)
                            title = SvValueGetString(titleV);
                        else
                            title = SVSTRING("");
                        
                        QBWindowContext movieDetailsCtx = QBVoDMovieDetailsContextCreate(ctx->appGlobals, (QBContentCategory) category, obj, ctx->provider, ctx->serviceId, ctx->serviceName, (SvObject) ctx->tree, ctx->categoryName, SvStringEqualToCString(ctx->recomendedFrom, "") ? title : ctx->recomendedFrom);
...
SvLocal void QBVoDMovieDetailsEnsureSimilarCarousel(QBVoDMovieDetails self)
{
    ...
    if (len > 1) {
        ...
        // { AYOUNG VOD3FakeCategories
        if (self->ctx->VOD3FakeCategoriesApply)
        {
            SvLogNotice("%s VOD3FakeCategories feature detected, will set the focus in similarCarousel", __func__);
...
SvLocal void QBVoDMovieDetailsTrailerPreviewPlay(QBVoDMovieDetailsTrailerPreview self)
{
    ...
    SvString trailerTitle = SvValueGetString((SvValue) SvDBRawObjectGetAttrValue(self->movieDetails->ctx->object, "name"));
    if (!trailerTitle)
        trailerTitle = SvStringCreate("", NULL);
    SvString productId = SVSTRING("");
    if ((SvDBObject)self->movieDetails->ctx->object)
    {
        productId = SvValueGetString(SvDBObjectGetID((SvDBObject)self->movieDetails->ctx->object));
    }
    BoldUsageLog(LOG_INFO, "\"VoDTrailerPlay\":{\"contentInfo\":\"%s/%s/%s\",\"vodId\":\"%s\"}",
                SvStringCString(self->movieDetails->ctx->serviceName), 
                SvStringCString(self->movieDetails->ctx->categoryName), 
                SvStringCString(trailerTitle), 
                SvStringCString(productId));
...
SvLocal void QBVoDMovieDetailsFillSideDetails(QBVoDMovieDetails self)
{
    ...
    bool isRental = SvStringEqualToCString(getPackageOfferType(self->ctx->object), "rental");
...
SvLocal bool QBVoDMovieDetailsInputEventHandler(SvWidget window, SvInputEvent e)
{
    ...
    if (ret) {
        ...
        if (self->ctx->VOD3FakeCategoriesApply)
        {
            SvLogNotice("%s VOD3FakeCategories feature detected, key up will be ignored", __func__);
        }
        else
        {
...
SvLocal void QBVoDMovieDetailsContextCreateWindow(QBWindowContext ctx_, SvApplication app)
{
    ...
    QBGlobalPlayerControllerStopCurrentController(ctx->appGlobals->playerController); // AMERELES Sacar DVB cuando se entra en en VoD3
    ...
    self->movieDetails.description = NULL;
    self->movieDetails.info = NULL;
    // AMERELES BEGIN - Add logo incon
    self->logoIcon = svSettingsWidgetCreate(app, "QBVoDMovieDetails.WindowLogo");
    svSettingsWidgetAttach(window, self->logoIcon, "QBVoDMovieDetails.WindowLogo", 1);
    // AMERELES END - Add logo incon
    ...
    // AMERELES BEGIN #2316 Categorización del título VoD
    bool isRental = SvStringEqualToCString(getPackageOfferType(self->ctx->object), "rental");
    BoldWidgetHide(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CategoriaIcon"), isRental);
    BoldWidgetHide(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Categoria"), isRental);
    BoldWidgetHide(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.PriceIcon"), !isRental);
    BoldWidgetHide(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.PricePromoIcon"), !isRental);
    BoldWidgetHide(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Price"), !isRental);
    
    svSettingsPushComponent("QBVoDMovieDetails.settings");
    if (isRental)
    {
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CategoriaIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.CategoriaIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Categoria"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Categoria", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.PriceIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.PriceIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.PricePromoIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.PricePromoIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Price"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Price", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.GenreIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.GenreIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Genre"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Genre", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CountryIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.CountryIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Country"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Country", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.DirectorIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.DirectorIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Director"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Director", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CastIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.CastIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Cast"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Cast", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.RatingsIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.RatingsIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Ratings"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Ratings", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.RuntimeIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.RuntimeIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Runtime"), SvStringCreate("QBVoDMovieDetails.MovieInfoRentals.Runtime", NULL));
    }
    else
    {
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CategoriaIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.CategoriaIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Categoria"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Categoria", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.PriceIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.PriceIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.PricePromoIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.PricePromoIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Price"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Price", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.GenreIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.GenreIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Genre"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Genre", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CountryIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.CountryIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Country"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Country", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.DirectorIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.DirectorIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Director"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Director", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.CastIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.CastIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Cast"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Cast", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.RatingsIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.RatingsIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Ratings"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Ratings", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.RuntimeIcon"), SvStringCreate("QBVoDMovieDetails.MovieInfo.RuntimeIcon", NULL));
        BoldWidgetMove(getSvWidgetChild(self->basicElements.movieInfo, "QBVoDMovieDetails.MovieInfo.Runtime"), SvStringCreate("QBVoDMovieDetails.MovieInfo.Runtime", NULL));
        
        //svSettingsSetInteger("QBVoDMovieDetails.MovieInfo.PriceIcon", "xOffset", -500);
    }
    svSettingsPopComponent();
    // AMERELES END #2316 Categorización del título VoD
    ...
    SvString title = QBMovieUtilsGetAttr(self->ctx->object, "name", SVSTRING(""));
    SvString productId = SVSTRING("");
    if ((SvDBObject)self->ctx->object)
    {
        productId = SvValueGetString(SvDBObjectGetID((SvDBObject)self->ctx->object));
    }
    BoldUsageLog(LOG_INFO, "\"MovieDetails\":{\"contentInfo\":\"%s/%s/%s\",\"vodId\":\"%s\"}",
                SvStringCString(ctx->serviceName), 
                SvStringCString(self->ctx->categoryName), 
                SvStringCString(title), SvStringCString(productId), 
                SvStringCString(productId));
    // END AMERELES #2131 tracking de navegación VOD en STBs híbridos
...
SvLocal void QBVoDDetailsShowSideMenu(QBVoDMovieDetails self)
{
    ...
    SvGenericObject contentInfo = QBVoDMovieDetailsContextCreateContentInfo((QBWindowContext) self->ctx);
    // AMERELES #2131 tracking de navegación VOD en STBs híbridos
    QBContentSideMenuSetContentInfo((QBContentSideMenu) self->contentSideMenu, contentInfo, self->ctx->categoryName, self->ctx->recomendedFrom);
...
QBWindowContext QBVoDMovieDetailsContextCreate(AppGlobals appGlobals, QBContentCategory category, SvDBRawObject object, SvGenericObject provider, SvString serviceId, SvString serviceName, SvObject externalTree, SvString categoryName, SvString recomendedFrom) // AMERELES #2131 tracking de navegación VOD en STBs híbridos
{
    ...
    self->categoryName = SVRETAIN(categoryName); // AMERELES #2131 tracking de navegación VOD en STBs híbridos
    self->externalProvider = SVRETAIN(provider);
    self->category = SVTESTRETAIN(category);
    self->movieDetails = NULL;
    self->recomendedFrom = SVRETAIN(recomendedFrom); // AMERELES #2131 tracking de navegación VOD en STBs híbridos
    ...
    self->VOD3FakeCategoriesApply = false; //VOD3FakeCategoriesApplyInThisMovie(appGlobals->vod3FakeCategories, self->object) &&
                                           //!BoldIsServiceWithMiLista(self->serviceId, self->serviceName);
