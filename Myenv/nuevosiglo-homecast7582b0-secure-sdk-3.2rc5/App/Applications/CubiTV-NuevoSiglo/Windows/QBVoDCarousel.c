...
QBMovieInfoFillProductWidgetInnov8on(QBMovieInfo self, SvDBRawObject metaData)
{
    ...
    if (self->ratings) {
        ...
        SvString extended_description = QBMovieUtilsGetAttr(metaData, "extended_description", SvStringCreate("", NULL));
        char str_char[1024];
        char *ptr = NULL;
    ...
    if (self->country) {
        ...
        SvString year = QBMovieUtilsCreateStrFromAttrInteger(metaData, "year", gettext("%i"), SVSTRING(""));
        SvString country = QBMovieUtilsGetAttr(metaData, "country", SvStringCreate("", NULL));
        SvStringBuffer buff = SvStringBufferCreate(NULL);
        ...
        SvString year_country = SvStringBufferCreateContentsString(buff, NULL);
