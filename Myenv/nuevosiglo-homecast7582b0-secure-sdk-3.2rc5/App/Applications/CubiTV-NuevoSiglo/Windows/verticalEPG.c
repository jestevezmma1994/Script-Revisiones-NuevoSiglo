...
VerticalEPGWindowInputEventHandlerImpl(EPGVerticalInfo info, const QBInputEvent *ev, bool direct)
{
    ...
    switch (keyCode) {
        ...
        case QBKEY_BACK: // AMERELES Fix en la navegación del menú de la derecha
        {
            if (QBVerticalEPGIsSideMenuShown((QBWindowContext) epg))
            {
                QBVerticalEPGHideSideMenu((QBWindowContext) epg);
                return true;
            }
        }
    }
