...
SvLocal void QBInitMinimalSetup(QBInit self, const char* executableName, const char *settingsPrefix)
{
    ...
    // AMERELES BEGIN Cache limit changed for 7580
    const char *sEnable = QBConfigGet("CACHE_LIMIT.ENABLED");
    if (sEnable && strcmp(sEnable, "enabled") == 0)
    {
        size_t softLimit = atoi(QBConfigGet("CACHE_LIMIT.SOFT")) * 1024 * 1024;
        size_t hardLimit = atoi(QBConfigGet("CACHE_LIMIT.HARD")) * 1024 * 1024;
        QBResourceManagerSetCacheSize(resourceManager, softLimit, hardLimit, NULL);
    }
    else
    {
    ...
    //QBResourceManagerSetCacheSize(resourceManager, QBCacheSizeGetResourceBundleSoftSize(), QBCacheSizeGetResourceBundleHardSize(), NULL);
    // AMERELES END Cache limit changed for 7580
...
SvLocal void QBInitCreateEverything(QBInit self)
{
    ...
    SvLogNotice ("%s Creating Fox Api controller",__func__);
    appGlobals->boldVodApi = BoldVodApiCreate(appGlobals);
    ...
    appGlobals->vod3FakeCategories = VOD3FakeCategoriesCreate(appGlobals);
...
SvLocal void QBInitStartEverything(QBInit self)
{
    ...
    if  (appGlobals->boldVodApi) {
        BoldVodApiStart(appGlobals->boldVodApi);
    }
...
//    }
    ...
    if  (appGlobals->vod3FakeCategories)
    {
        VOD3FakeCategoriesStart(appGlobals->vod3FakeCategories);
    }

    // AMERELES Buscador Global
    appGlobals->vodGridBrowserInitAsSearch = false;
    ...
    //  { NR: Initialize BoldUsageLogFeature
    BoldUsageLogInitialize(appGlobals);
    // }
...
void QBInitFinishApplication(AppGlobals appGlobals, bool terminate)
{
    ...
    SVTESTRELEASE(appGlobals->boldVodApi);
    appGlobals->boldVodApi = NULL;
