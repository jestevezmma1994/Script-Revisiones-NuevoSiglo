        ...
        case QBKEY_YELLOW:
            {
                QBCarouselMenuItemService menu = (QBCarouselMenuItemService) 
                    QBProvidersControllerServiceGetServiceByID(appGlobals->providersController, SVSTRING("VOD"), getBoldSearchServiceID());
                if (!menu || !QBCarouselMenuPushContext(menu, QBCarouselMenuGetOwnerTree(menu)))
                {
                    nodeID = SVSTRING("SEARCHBOLD");
                }
                else
                {
                    appGlobals->vodGridBrowserInitAsSearch = true;
                }
            }
            break;
   ...
   for (int i = 0; i < cnt; i++) {
      ...
      if (QBPlatformOutputEnable(i, outputs[i].mode, outputs[i].aspectRatio) < 0) {
         if (i == (int) outputs[i].masterID) {
          ...
          if (!HDCPENABLE || (HDCPENABLE && strcmp(HDCPENABLE, "enabled") == 0))
          {
              QBPlatformSetHDCP(i, QBContentProtection_required, QBPlatformOutputPriority_application);
          }
          else
          {
              QBPlatformSetHDCP(i, QBContentProtection_disable, QBPlatformOutputPriority_application);
          }
