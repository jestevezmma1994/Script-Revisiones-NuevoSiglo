...
struct QBInnov8onFetchService_t {
    ...
    bool middlewareManagerFromProxy;       // AMERELES [2986] Funcionalidad Mi Lista
    SvString lastOperation;                // AMERELES [2986] Funcionalidad Mi Lista
...
QBInnov8onFetchService__dtor__(void *_self)
{
    ...
    if (self->middlewareManager && self->middlewareManagerFromProxy)
    {
        QBMiddlewareManagerStop(self->middlewareManager);
                                               ...
                                               const QBContentMgrTransactionStatus * const status)
{
    ...
    if(status->status.state == QBRemoteDataRequestState_finished) {
        ...
        SVTESTRELEASE(self->lastOperation);
        self->lastOperation = SVSTRING("");
...
QBInnov8onFetchServiceStart(QBInnov8onFetchService self, SvGenericObject product, SvScheduler scheduler)
{
    ...
    // AMERELES [2986] Funcionalidad Mi Lista
    SVTESTRELEASE(self->lastOperation);
    self->lastOperation = SVSTRING("NORMAL");
    SVRELEASE(req);
...
// BEGIN AMERELES [2986] Funcionalidad Mi Lista
void
BoldQBInnov8onFetchServiceStartMiListaAdd(QBInnov8onFetchService self, SvGenericObject product, SvScheduler scheduler, bool toAdd)
{
    if (unlikely(!self || !product)) {
        SvLogError("%s NULL argument passed", __func__);
        return;
    ...
    SvString operation = toAdd ? SVSTRING("stb.MiListaAdd") : SVSTRING("stb.MiListaRemove");
    
    SvXMLRPCRequest req = SvXMLRPCRequestCreateWithJSONTemplate(server, operation, NULL,
                                                                "[%s, \"\", [%s], %d, %d, [\"\"], true]",
                                                                SvStringCString(mac), SvStringCString(searchCriteria), 0, 1);
    SVRELEASE(searchCriteria);
    if (unlikely(!req)) {
    ...
    self->transaction = Innov8onProviderTransactionCreate(req, 15, NULL);
    self->lastOperation = SVSTRING("MILISTA");
    SVRELEASE(req);
    SvInvokeInterface(QBContentMgrTransaction, self->transaction, setListener, (SvGenericObject) self, NULL);
    SvInvokeInterface(QBContentMgrTransaction, self->transaction, start, scheduler);
    SvLogNotice("%s Refresh product id=%s (pane=%p)", __func__, id ? SvStringCString(id) : "--", self);
...
QBInnov8onLoadablePaneActivityEnded(SvObject self_, QBContentCategory category)
{
    ...
    } else if (self->product) { // AMERELES Problema al querer reproducir VoD "Ahora en TV"
        self->callback(self->callbackData, self, self->product); // AMERELES Problema al querer reproducir VoD "Ahora en TV"
    } else {
        self->callback(self->callbackData, self, NULL);
    }
}
                                   ...
                                   QBLoadablePaneState state)
{
    ...
    if ((state == QBLoadablePaneStateShown || state == QBLoadablePaneStateStarted) &&
        self->refreshParams) {
        // AMERELES [#2094] PROXY de Presentación
        // AMERELES [2986] Funcionalidad Mi Lista
        // [#4049]  Problema al reproducir desde Mi Lista un VOD previamente alquilado
        //if (self->category) {
        if (self->category && self->fetchService && self->fetchService->lastOperation && 
            SvStringEqualToCString(self->fetchService->lastOperation, "MILISTA")) {
...
QBInnov8onLoadablePaneInit(QBInnov8onLoadablePane self,
                           ...
                           SvString serviceID) // AMERELES [2986] Funcionalidad Mi Lista
{
    int level = 1;
    ...
    // BEGIN AMERELES [2986] Funcionalidad Mi Lista
    if (BoldIsMWProxyService(serviceID))
    {
        const char *mwPROXYurl = QBConfigGet("mwPROXY.URL");
        SvString prefixUrl = SvStringCreate(mwPROXYurl ? mwPROXYurl : "", NULL);
        SvString cmdLineUrl = SvStringCreate(mwPROXYurl ? mwPROXYurl : "", NULL);
        ...
        QBMiddlewareManager middlewareManager = QBMiddlewareManagerCreate();
        QBMiddlewareManagerInit(middlewareManager, &middlewareManagerParams);
        SVRELEASE(cmdLineUrl);
        QBMiddlewareManagerStart(middlewareManager, appGlobals->smartCardMonitor);
        self->fetchService = QBInnov8onFetchServiceCreate(middlewareManager, QBInnov8onLoadablePaneFetchEnded, self);
        self->fetchService->middlewareManagerFromProxy = true;
    }
    else
    {
        self->fetchService = QBInnov8onFetchServiceCreate(appGlobals->middlewareManager, QBInnov8onLoadablePaneFetchEnded, self);
        self->fetchService->middlewareManagerFromProxy = false;
    }
    // END AMERELES [2986] Funcionalidad Mi Lista
    //self->fetchService = QBInnov8onFetchServiceCreate(appGlobals->middlewareManager, QBInnov8onLoadablePaneFetchEnded, self);
