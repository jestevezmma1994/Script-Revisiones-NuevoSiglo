...
QBInnov8onExtendedInfoSetObject(SvWidget w, SvGenericObject object)
{
    ...
    if (movie) {
        ...
        if (description_)
        {
            // BEGIN AMERELES Agregar Información de Lenguaje a contenidos de VoD3, ahora extended_description es "extended_description"|"language"
            char str_char[1024];
            char *ptr = NULL;
            
            strncpy(str_char, description_, 1023);
            strtok_r (str_char, "|", &ptr);
            
            if (ptr && strcmp(ptr, "") != 0)
            {
                description = SvStringCreate(str_char, NULL);
            }
            ...
            //description = SvStringCreate(description_, NULL);
            // END AMERELES Agregar Información de Lenguaje a contenidos de VoD3, ahora extended_description es "extended_description"|"language"
        }
