...
QBVoDMenuPurchaseInnov8onOffer(QBVoDMenu self, SvDBRawObject product, SvDBObject offer_)
{
    ...
    if (offerIdV) {
        ...
        self->dialog = QBConfCondAuthDialogCreate(appGlobals, authenticator,
                                          ...
                                          0, true, NULL, NULL,
                                          gettext("OK"), gettext("Cancel"), purchaseAuth, false);
...
QBVoDMenuPlayContent(QBVoDMenu self, SvDBRawObject movie, bool authenticated)
{
    ...
    //QBMediaFileType type = QBAppKitURIToMediaFileType(URI, NULL); // BOLDMSS Proveedores con proxy
    SvContent c = NULL;
    if (URI)
    {
        type = QBAppKitURIToMediaFileType(URI, NULL);
        //SvLogNotice("URI = %s", SvStringCString(URI)); // BOLDMSS Proveedores con proxy
        SvLogNotice("%s: URI = %s", __func__, SvStringCString(URI));
    }
    ...
    if (self->BoldMovieCityParam)
    {
        ...
        switch (serviceType)
        {
            case BoldVodServiceFOX_hls:
                SVRELEASE(URI);
                URI = SVRETAIN(self->BoldMovieCityParam);
                SvLogNotice("%s : MovieCitiy HLS URI = %s",__func__, SvStringCString(URI));
            break;
            case BoldVodServiceFOX_ss:
                SvLogNotice("%s : FOXv2 Service", __func__);
                SVRELEASE(URI);
                URI = SVRETAIN(removePidFromUri(URIstr));
            ...
            case BoldVodServiceFOX_ss_v3:
                ...
                SVRELEASE(URI);
                URI = SVRETAIN(BoldVodFoxGetLink(self->appGlobals->boldVodApi));
                ssl = (SvSSLParams) SvTypeAllocateInstance(SvSSLParams_getType(), &error);
                SvSSLParamsInit(ssl, SvSSLVersion_TLSv1, NULL);
                SvSSLParamsSetVerificationOptions(ssl, false, false, NULL, NULL);
                QBAnyPlayerLogicSetPlayReadyLicenceRequestSSLParams(anyPlayerLogic, ssl);
            ...
            case BoldVodServiceHBO:
                ...
                SVRELEASE(URI);
                URI = SVRETAIN(BoldVodHboGetLink(self->appGlobals->boldVodApi));
                ssl = (SvSSLParams) SvTypeAllocateInstance(SvSSLParams_getType(), &error);
                SvSSLParamsInit(ssl, SvSSLVersion_TLSv1, NULL);
                SvSSLParamsSetVerificationOptions(ssl, false, false, NULL, NULL);
                QBAnyPlayerLogicSetPlayReadyLicenceRequestSSLParams(anyPlayerLogic, ssl);
            ...
            case BoldVodServiceTurner:
                ...
                SVRELEASE(URI);
                URI = SVRETAIN(BoldVodTurnerGetLink(self->appGlobals->boldVodApi));
                ssl = (SvSSLParams) SvTypeAllocateInstance(SvSSLParams_getType(), &error);
                SvSSLParamsInit(ssl, SvSSLVersion_TLSv1, NULL);
                SvSSLParamsSetVerificationOptions(ssl, false, false, NULL, NULL);
                QBAnyPlayerLogicSetPlayReadyLicenceRequestSSLParams(anyPlayerLogic, ssl);
...
QBVoDMenuPlay_(void * self_, SvString param)
{
    QBVoDMenu self = self_; // BOLDMSS Proveedores con proxy
    ...
    //QBVoDMenuHide(self); // BOLDMSS Proveedores con proxy
    
    self->play_in_progress = false; // BOLDMSS Proveedores con proxy
}
// END BOLDMSS Proveedores con proxy
...
QBVoDMenuPlay(void *self_, SvString id, QBBasicPane pane, QBBasicPaneItem item)
{
    ...
    if (!self->play_in_progress)
    {
        ...
        BoldVodProductHandler(self->appGlobals->boldVodApi, self->product, (void*)self, &QBVoDMenuPlay_);
    }
    /////////// BOLDMSS //////////////
}
...
QBVoDMenuStart(QBVoDMenu self, SvDBRawObject product, SvObject path)
{
    ...
    if (!strcmp(type, "vod")) {
        ...
        self->pane = pane;
        QBInnov8onLoadablePaneInit(pane, self->ctxMenu, self->appGlobals, self->serviceId); // AMERELES [2986] Funcionalidad Mi Lista
        svSettingsPopComponent();
...
QBVoDMenuRentSuccessfulPopupCallback(void *self_, SvWidget dialog, SvString buttonTag, unsigned keyCode)
{
    ...
    if (self->purchase) {
        //QBVoDMenuPlay_(self); // BOLDMSS Proveedores con proxy
        QBVoDMenuPlay_(self, NULL);
        SVTESTRELEASE(self->purchase);
        self->purchase = NULL;
