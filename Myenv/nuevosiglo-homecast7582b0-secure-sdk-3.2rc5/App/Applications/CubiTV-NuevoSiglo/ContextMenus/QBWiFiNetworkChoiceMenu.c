...
SvLocal void QBWiFiNetworkChoiceItemFillTitle(void *item_, QBTextRendererTask task)
{
    ...
    item->title->off_x = item->captionXPos + task->box.x + 42; // AMERELES [#3423] SDK 3.0 rc5 - Solapamiento en gráficos
    //item->title->off_y = (item->box->height + item->mboxHeight) / 2 - 1.6 * task->box.y;
    item->title->off_y = (item->box->height + item->mboxHeight) / 2 - title->height; // AMERELES [#3423] SDK 3.0 rc5 - Solapamiento en gráficos
...
SvLocal void QBWiFiNetworkChoiceItemFillSubtitle(void *item_, QBTextRendererTask task)
{
    ...
    item->subtitle->off_x = item->captionXPos + task->box.x + 42; // AMERELES [#3423] SDK 3.0 rc5 - Solapamiento en gráficos
    //item->subtitle->off_y = (item->box->height + item->mboxHeight) / 2 + 0.1 * task->box.y;
    item->subtitle->off_y = item->box->height - item->subtitle->height - 2; // AMERELES [#3423] SDK 3.0 rc5 - Solapamiento en gráficos
...
SvLocal SvWidget QBWiFiNetworkChoiceControllerCreateItem(SvObject self_, SvObject node, SvObject path, SvApplication app, XMBMenuState initialState)
{
    ...
    QBFrameConstructData params = {
        ...
        .height = self->height,  // AMERELES [#3423] SDK 3.0 rc5 - Solapamiento en gráficos
    ...
    if (SvObjectIsInstanceOf(node, QBWiFiNetwork_getType())) {
        ...
        svWidgetAttach(box, stateIcon, 0, (box->height - signalIcon->height) / 3, 4);
