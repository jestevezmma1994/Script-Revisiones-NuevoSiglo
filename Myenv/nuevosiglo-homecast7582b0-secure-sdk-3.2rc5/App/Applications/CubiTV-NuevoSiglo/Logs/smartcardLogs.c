...
SvLocal void QBSmartcardLogGotInfo(void *self_, const QBCASCmd cmd, int status, QBCASInfo info)
{
    ...
    //BoldUsageLogUpdateIds((!status && card_number && self->state == QBCASSmartcardState_correct) ? card_number : SVSTRING("unknown"), (!status && info->chip_id) ? info->chip_id : SVSTRING("unknown"));
    //BoldContentTrackingUpdateIds((!status && card_number && self->state == QBCASSmartcardState_correct) ? card_number : SVSTRING("unknown"));
