...
struct QBDiagnosticsWidgetElement_t {
    ...
    SvWidget label, text;
    SvWidget percentage; // AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
};
...
// BEGIN AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
SvLocal void
QBDiagnosticsWidgetElementSetPercentage(QBDiagnosticsWidgetElement self, const char* percentage)
{
    SvString val = NULL;
    if (self && self->percentage) {
        val = SvStringCreate((percentage ? gettext(percentage) : gettext("N/A")), NULL);
        QBAsyncLabelSetText(self->percentage, val);
        SVRELEASE(val);
    }
}
// END AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
...
QBDiagnosticsWidgetSetSignalLevelBar(QBDiagnosticsWidget self)
{
    ...
    if (carrier_lock > 0) {
        char *signalLevel = NULL;
        asprintf(&signalLevel, "%d%%", QBDiagnosticsGetSignalStrength(self->diagnostics));
        
        if (signalLevel) {
            QBDiagnosticsWidgetElementSetPercentage(element, signalLevel);
            free(signalLevel);
        }
    } else {
        QBDiagnosticsWidgetElementSetPercentage(element, "0%");
    }
    // END AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
}
...
QBDiagnosticsWidgetSetQualityLevelBar(QBDiagnosticsWidget self)
{
    ...
    int carrier_lock = QBDiagnosticsGetCarrierLock(self->diagnostics);
    if (carrier_lock > 0) {
        char *qualityLevel = NULL;
        asprintf(&qualityLevel, "%d%%", QBDiagnosticsGetQualityLevel(self->diagnostics));
        if (qualityLevel) {
            QBDiagnosticsWidgetElementSetPercentage(element, qualityLevel);
            free(qualityLevel);
        }
    } else {
        QBDiagnosticsWidgetElementSetPercentage(element, "0%");
                               ...
                               const char* id)
{
    ...
    if (id && (strcmp(id, "signalLevelBar") == 0 || strcmp(id, "qualityLevelBar") == 0)) {
        ...
        if (self->useRatioSettings) {
            ...
            // BEGIN AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
            snprintf(buf, 256, "%s.widget.GaugePercentage", self->name);
            element->percentage = QBAsyncLabelNewFromRatioSettings(app, element->w->width, element->w->height, buf,
                                                             renderer, NULL);
            if (element->percentage) {
                QBDiagnosticsWidgetElementSetPercentage(element, "0%");
                svSettingsWidgetAttachWithRatio(element->w, element->percentage, buf, 1);
            }
            // END AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
        } else {
            ...
            // BEGIN AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
            snprintf(buf, 256, "%s.widget.GaugePercentage", self->name);
            element->percentage = QBAsyncLabelNew(app, buf, renderer);
            if (element->percentage) {
                QBDiagnosticsWidgetElementSetPercentage(element, "0%");
                svSettingsWidgetAttach(element->w, element->percentage, buf, 1);
            }
            // END AMERELES [#3338] Agregar porcentaje de señal en Estado de Canal
        }
    } else {
