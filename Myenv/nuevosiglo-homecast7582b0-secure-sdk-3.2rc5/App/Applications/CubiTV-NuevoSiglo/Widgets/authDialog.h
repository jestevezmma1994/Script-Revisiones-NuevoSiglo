...
// AMERELES Support authentication dialog with no-pin requirement (purchase VoD without PIN)
/**
 * Creates a new instance of QBAuthDialog. To show it on screen call QBDialogRun().
 * @param[in] appGlobals         CubiTV Application
 * @param[in] authenticator      object implementing QBAuthDialogAuthenticator interface
 ...
 * @param[in] local              true if popup should be a QBLocalWindow, false if a QBGlobalWindow
 * @param[in] globalName         string representing name of the QBGlobalWindow (valid only if local is set to false)
 * @param[in] text_button_ok     text of Accept button
 * @param[in] text_button_cancel text of Decline button
 * @param[in] requireAuth        true if requires pin to continue, needAuth
 * @param[in] focusOK            true if dialog needs to focus on Accept button
 * @param[out] helperPtr    internal pointer to widget that implements QBAuthDialog behaviour
 * @return widget representing new instance of QBAuthDialog
 **/
SvWidget QBConfCondAuthDialogCreate(AppGlobals appGlobals, SvGenericObject authenticator, const char *title, const char *message, bool local, SvString globalName, SvWidget *helperPtr, const char *text_button_ok, const char *text_button_cancel, bool requireAuth, bool focusOK);
