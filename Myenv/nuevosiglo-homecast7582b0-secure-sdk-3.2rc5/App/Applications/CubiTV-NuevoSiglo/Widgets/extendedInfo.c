...
struct QBExtendedInfo_t {
    SvWidget text, scrollBar, viewPort, title, time, playerInfoLabel; // AMERELES [#2406] Agregar información de Bitrate al reproducir VoD
...
void
QBExtendedInfoSetPlayerInfoLabelText(SvWidget w, SvString playerInfoLabel)
{
    QBExtendedInfo prv = w->prv;
    if (!prv->playerInfoLabel)
        return;
    
    if(!playerInfoLabel)
        playerInfoLabel = SVSTRING("");
    
    QBAsyncLabelSetText(prv->playerInfoLabel, playerInfoLabel);
}
...
SvWidget QBExtendedInfoNew(SvApplication app, const char *name, AppGlobals appGlobals)
{
    ...
    // AMERELES [#2406] Agregar información de Bitrate al reproducir VoD
    snprintf(buf, sizeof(buf), "%s.%s", name, "playerInfoLabel");
    
    if (svSettingsIsWidgetDefined(buf))
    {
        prv->playerInfoLabel = QBGridAddAsyncLabel(prv->grid, buf, renderer);
    }
