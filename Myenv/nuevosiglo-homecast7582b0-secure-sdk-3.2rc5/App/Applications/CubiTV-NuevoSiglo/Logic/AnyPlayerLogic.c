...
SvLocal SvObject QBAnyPlayerLogicCreateInnov8onController(QBAnyPlayerLogic self, SvObject product)
{
    ...
    plugin = (SvObject) ColumbusQBPlayerControllerPluginGetInstance();
    if (plugin) {
        QBVoDAnyPlayerControllerAddPlugin((QBVoDAnyPlayerController) controller, plugin);
    }
    ...
    if (self->showDialog) {
        ...
        switch (eventID) {
            ...
            case QBAnyPlayerControllerEventID_error:
                ...
                // AMERELES [#2234] Mensaje en popup de error de reproducción VOD - versión comercial
                SvString errorMsg = SvStringCreateWithFormat("%s\n%s", data ? SvStringCString((SvString) data) : " ", message);
...
{
    // AMERELES [#2234] Mensaje en popup de error de reproducción VOD - versión comercial (code * (-1))
    if (code <= SV_ERR_INIT_DECODER && code > SV_ERR_DRM_INIT_FAILURE) {
        return SvStringCreateWithFormat(gettext("Decoder error: %d"), code * (-1));
    } else if (code <= SV_ERR_DRM_INIT_FAILURE && code > SV_ERR_SERVER_CONN_FAILED) {
        return SvStringCreateWithFormat(gettext("DRM error: %d"), code * (-1));
    } else if (code <= SV_ERR_SERVER_CONN_FAILED && code > SV_ERR_CPC_XML_SYNTAX_ERROR) {
        switch (code) {
            case SV_ERR_SERVER_FILE_NOT_FOUND:  return SvStringCreateWithFormat(gettext("File not found: %d"), code * (-1));
            case SV_ERR_UNSUPPORTED_FORMAT:     return SvStringCreateWithFormat(gettext("Format not supported: %d"), code * (-1));
            case SV_ERR_UNSUPPORTED_PROTOCOL:   return SvStringCreateWithFormat(gettext("Protocol not supported: %d"), code * (-1));
            case SV_ERR_BAD_URL:                return SvStringCreateWithFormat(gettext("Bad URL: %d"), code * (-1));
            case SV_ERR_BAD_SERVER_RESPONSE:    return SvStringCreateWithFormat(gettext("Bad server response: %d"), code * (-1));
            case SV_ERR_UNEXPECTED_SERVER_RESPONSE: return SvStringCreateWithFormat(gettext("Unexpected server response: %d"), code * (-1));
            default:                            return SvStringCreateWithFormat(gettext("Server communication error: %d"), code * (-1));
        }
