...
#define DVB_CHANNELS_RESTORE_TIMEOUT_MS 200
#define IP_CHANNELS_RESTORE_TIMEOUT_MS 2000
#define EMMS_WAITER_TIMEOUT_S 60             // EMMs waiter timeout in seconds

#define EPG_REFRESH_PERIOD_S 1 * 60 * 60 // send GetEPG requests every 1 hour
#define EPG_REFRESH_PERIOD_RANDOM_PART_S 30 * 60 // randomize sending GetEPG requests within 30 minutes
...
struct QBInitLogic_t {
    ...
    // AMERELES Channel Logos from Remote Server
    SvFiber cmdUpdateLogosFiber;
    SvFiberTimer cmdUpdateLogosTimer;
    QBShellPipe cmdUpdateLogosPipe;
    time_t cmdUpdateLogosTimeStamp;
};
...
SvLocal void QBInitLogicAdaptService(SvObject self_, SvObject service)
{
    ...
    // BEGIN AMERELES Channel Logos from Remote Server
    QBInitLogic self = (QBInitLogic) self_;
    
    if (time(NULL) - self->cmdUpdateLogosTimeStamp  > 2 * 3600)
    {
        self->cmdUpdateLogosTimeStamp = time(NULL);
        BoldInitLogicUpdateLogos(self);
    }
    // END AMERELES Channel Logos from Remote Server
}
...
QBInitLogic QBInitLogicCreate(AppGlobals appGlobals)
{
    ...
    // AMERELES Channel Logos from Remote Server
    self->cmdUpdateLogosFiber = NULL;
    self->cmdUpdateLogosTimer = NULL;
    self->cmdUpdateLogosPipe = NULL;
    self->cmdUpdateLogosTimeStamp = 0;
...
void QBInitLogicPostInit(QBInitLogic self, SvHashTable whitelist)
{
    ...
    SvSSLParamsSetVerificationOptions(sslParams, true, true, "/usr/share/ca-certificates/", NULL);
    QBServiceRegistry registry = QBServiceRegistryGetInstance();
    QBResourceManager resourceManager = (QBResourceManager) QBServiceRegistryGetService(registry, SVSTRING("ResourceManager"));
    QBResourceManagerSetSSLParams(resourceManager, sslParams, NULL);
    SVRELEASE(sslParams);
}
...
{
    ColumbusQBPlayerControllerPluginClenUp();
}
...
SvLocal unsigned int QBInitLogicGenerateRandomTimeRangeAlignment(void)
{
    return (EPG_REFRESH_PERIOD_S + (rand() % EPG_REFRESH_PERIOD_RANDOM_PART_S) - EPG_REFRESH_PERIOD_RANDOM_PART_S / 2);
}
...
void QBInitLogicCreateEPGManager(QBInitLogic self)
{
    ...
    unsigned int timeRangeAlignment = QBInitLogicGenerateRandomTimeRangeAlignment();
    SvLogNotice("Generated time range alignment = %u", timeRangeAlignment);
    SvEPGManagerSetTimeRangeAlignment(epgManager, timeRangeAlignment);
    SvInvokeInterface(SvChannelMerger, self->appGlobals->channelMerger, addListener, (SvObject) self->appGlobals->mapper, NULL);
    ...
    if (self->appGlobals->epgPlugin.IP.channelList) { 
        int pluginID = SvInvokeInterface(SvEPGChannelListPlugin, self->appGlobals->epgPlugin.IP.channelList, getID);
        SvInvokeInterface(SvChannelMerger, self->appGlobals->channelMerger, setPreferredPluginID, pluginID, NULL); 
    }
    ...
    SvEPGManagerSetTimeLimit(epgManager, -1 * 24 * 3600, 3 * 24 * 3600, NULL);
    SvLogNotice("SvEPGManagerSetTimeLimit -1 * 24");
...
void QBInitLogicCreatePlaylists(QBInitLogic self)
{
    ...
    SvValue isInternalValue = SvValueCreateWithBoolean(true, NULL);
    QBPlaylistManagerSetPlaylistMetaData(playlists, (SvObject) channelView, (SvObject) SVSTRING("isInternal"), (SvObject) isInternalValue);
    SVRELEASE(isInternalValue);
    SVRELEASE(channelView);
...
void QBInitLogicCreateMWClientServices(AppGlobals appGlobals)
{
    ...
    if (columbusQBPlayerControllerPlugin) {
        ...
        QBConaxPlayReadySetHelperService(conaxPlayReady, columbusQBPlayerControllerPlugin, NULL);
    }
    ...
    CubiwareMWClientServiceConaxPlugin conaxPlugin = CubiwareMWClientServiceCreateConaxPlugin(appGlobals->scheduler,
                                                                                              ...
                                                                                              CubiwareMWClientServiceConaxPluginMode_RequestID);
    if (conaxPlugin && columbusQBPlayerControllerPlugin) {
        ColumbusQBPlayerControllerPluginSetHelper(columbusQBPlayerControllerPlugin, (SvObject) conaxPlugin, NULL);
...
void QBInitLogicLoadHlsSSLAuthParams(QBInitLogic self)
{
    ...
    SvSSLParams sslParams = SvSSLParamsCreateDefault(NULL);
    SvSSLParamsSetVerificationOptions(sslParams,
                                      true, true,
                                      "/usr/share/ca-certificates",
                                      NULL);
    if (sslParams) {
        QBGlobalStorageSetItemByName(gs, SVSTRING("SSLParams"), (SvObject) sslParams ,NULL);
        SVRELEASE(sslParams);
    }
}
...
// BEGIN AMERELES Channel Logos from Remote Server
SvLocal void 
BoldInitLogicUpdateLogosStep(void *self_)
{
    QBInitLogic self = self_;
    SvFiberDeactivate(self->cmdUpdateLogosFiber);
    SvFiberEventDeactivate(self->cmdUpdateLogosTimer);

    if (self->cmdUpdateLogosPipe)
    {
        int ret = QBShellPipeHasAsyncResult(self->cmdUpdateLogosPipe);
        if (ret != 0)
        {
            SvLogNotice("%s(): Actualization of channel logos from server (END)", __func__);
            QBShellPipeClose(self->cmdUpdateLogosPipe);
            self->cmdUpdateLogosPipe = NULL;
            
            // Remove cache to actualize logos
            QBServiceRegistry registry = QBServiceRegistryGetInstance();
            QBResourceManager resourceManager = (QBResourceManager) QBServiceRegistryGetService(registry, SVSTRING("ResourceManager"));
        ...
        {
            SvLogNotice("%s(): Actualization of channel logos from server (waiting to close pipe)", __func__);
            SvFiberTimerActivateAfter(self->cmdUpdateLogosTimer, SvTimeFromMs(250));
        }
    }
}

void BoldInitLogicUpdateLogos(QBInitLogic self)
{
    const char *tmp = QBConfigGet("LOGOS_IN_SERVER.ENABLED");
    if (tmp && strcmp(tmp, "enabled") == 0)
    {
        SvLogNotice("%s(): Actualization of channel logos from server (BEGIN)", __func__);
        //QBShellExec("update_channel_logos");
        
        if (self->cmdUpdateLogosPipe)
            QBShellPipeClose(self->cmdUpdateLogosPipe);
        
        if (!self->cmdUpdateLogosFiber)
        {
            self->cmdUpdateLogosFiber = SvFiberCreate(SvSchedulerGet(), NULL, "BoldInitLogicUpdateLogos",
                                           (SvFiberStepFun*) &BoldInitLogicUpdateLogosStep, self);
            self->cmdUpdateLogosTimer = SvFiberTimerCreate(self->cmdUpdateLogosFiber);
        }
        
        self->cmdUpdateLogosPipe = QBShellPipeOpen("update_channel_logos", NULL);
        
        if (QBShellPipeRequestAsyncResult(self->cmdUpdateLogosPipe) != 0)
        {
            SvLogNotice("%s(): Actualization of channel logos from server (END)", __func__);
            QBShellPipeClose(self->cmdUpdateLogosPipe);
            self->cmdUpdateLogosPipe = NULL;
        ...
        {
            SvLogNotice("%s(): Actualization of channel logos from server (waiting to close pipe)", __func__);
            SvFiberTimerActivateAfter(self->cmdUpdateLogosTimer, SvTimeFromMs(1000));
        }
    }
}
// END AMERELES Channel Logos from Remote Server
