...
EventsLogicDescShouldBeBlocked(EventsLogic self, SvEPGEvent event)
{
    ...
    if (PC_HIDE_TITLES && (strcmp(PC_HIDE_TITLES, "ALWAYS") == 0 || strcmp(PC_HIDE_TITLES, "FOLLOW_PC") == 0))
    {
        return QBParentalControlEventShouldBeBlocked(self->pc, event) && !QBParentalControlLogicIsAuthenticated(self->pcLogic);
    }
    else if (PC_HIDE_TITLES && strcmp(PC_HIDE_TITLES, "NEVER") == 0)
    {
        return false;
    }
