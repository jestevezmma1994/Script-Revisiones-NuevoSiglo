...
struct QBOperateScanningLogic_ {
    ...
    /// < GRIAL: PERIODIC BACKGROUND SCANNER
    SvFiber fiber;
    SvFiberTimer timer;
    int     period;
    bool	started;
...
SvLocal void PeriodicBackgroundScannerStep(void *self_);
...
SvLocal void QBOperateScanningLogicDestroy(void *self_)
{
    ...
    if(self->fiber)                     /// < Free memory use for periodic BkgScanner
	    SvFiberDestroy(self->fiber);    /// < Free memory use for periodic BkgScanner
    SVTESTRELEASE(self->conf);          /// < Free memory use for periodic BkgScanner
...
QBOperateScanningLogic QBOperateScanningLogicCreate(AppGlobals appGlobals)
{
    ...
    /// < GRIAL: PERIODIC BACKGROUND SCANNER
    const char *tmp;
    if((tmp = QBConfigGet("PERIODICSCANNING.ENABLED")) && 
        strncmp(tmp, "enabled", strlen("enabled")) == 0)
        {
        
        SvLogNotice("\n%s():%i PERIODICSCANNING enabled\n",__func__,__LINE__);
        /**
         * Set fiber
        */
        if (!self->fiber) {

            SvLogNotice("\n%s():%i Creating BkgScanner Fiber\n",__func__,__LINE__);
    	    
            self->fiber = SvFiberCreate(appGlobals->scheduler, NULL, "PeriodicBackgroundScanner", PeriodicBackgroundScannerStep, self);
            self->timer = SvFiberTimerCreate(self->fiber);
            self->started = true;            
            self->period = (QBConfigGet("PERIODICSCANNING.PERIOD")) ? atoi(QBConfigGet("PERIODICSCANNING.PERIOD")) : 3600;
            self->conf = NULL;
                         
            SvLogNotice("\n%s():%i BkgScanner Period = %i seg\n",__func__,__LINE__,self->period);
            
            SvFiberTimerActivateAfter(self->timer, SvTimeConstruct(self->period, 0));
        }
	...
	if( self->started )
    {
        ...
        */
        if(!self->conf){
            QBChannelScanningConfManager channelScanningConf = (QBChannelScanningConfManager) QBServiceRegistryGetService(QBServiceRegistryGetInstance(),
                                                                                                                  SVSTRING("QBChannelScanningConfManager"));
            self->conf = QBChannelScanningConfManagerGetConf(channelScanningConf, SVSTRING("background"));
        }
        
        //SvLogNotice("\nGRIAL %s():%i Calling bkg\n",__func__,__LINE__);
        
        self->callback = NULL;
        BackgroundScannerSetCallback(self->scanner, QBOperateScannignLogicBackgroundScannerCallback, self);
        BackgroundScannerStartScanning(self->scanner, tunerNum,self->conf);
        /**
         * Reactive after period
        */
        SvFiberTimerActivateAfter(self->timer, SvTimeConstruct(self->period, 0));
        SvLogNotice("%s():%i: BkgScanner Fiber reactivated",__func__,__LINE__);

    } else {

        SvLogNotice("%s():%i: BkgScanner Fiber not started",__func__,__LINE__);
