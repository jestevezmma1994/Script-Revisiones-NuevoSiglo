...
/*
 * ar.h : archive file defintions
 */


/* the rather well known ar format */
#define ARMAG	"!<arch>\n"
#define SARMAG	8
#define ARFMAG	"`\n"
...
struct ar_hdr {
    char	ar_name[16];	/* terminated with a '/' */
    char	ar_date[12];	/* secs from UNIX epoch in decimal digits */
    char	ar_uid[6];	/* in decimal digits */
    char	ar_gid[6];	/* in decimal digits */
    char	ar_mode[8];	/* in octal digits */
    char	ar_size[10];	/* in decimal digits */
    char	ar_fmag[2];	/* the ARFMAG string */
};
