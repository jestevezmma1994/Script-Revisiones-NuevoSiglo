...
extern QBMp4Indexer *
QBMp4IndexerCreate(const QBMp4Info *info);

extern void
QBMp4IndexerDestroy(QBMp4Indexer *indexer);

/// @returns 1  when is ready
/// @returns 0  when processing is ongoing
/// @returns <0  error
extern int
QBMp4IndexerProcess(QBMp4Indexer *indexer);

/// reset position on all tracks
/// iff @a pts90k is beyond seeking range, @a indexer will go to eos-state
extern void
QBMp4IndexerSeek(QBMp4Indexer *indexer, uint64_t pts90k);

extern uint64_t
QBMp4IndexerGetStartPosition(const QBMp4Indexer *indexer);
extern uint64_t
QBMp4IndexerGetStartOffset(const QBMp4Indexer *indexer);
...
extern bool
QBMp4IndexerGetNextSample(QBMp4Indexer *indexer, QBMp4IndexPoint *point_out);
