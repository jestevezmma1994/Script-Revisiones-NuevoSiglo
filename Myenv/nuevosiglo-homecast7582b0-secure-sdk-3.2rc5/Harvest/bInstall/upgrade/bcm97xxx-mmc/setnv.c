...
int enc_write(int fd, void *buffer, int wsize)
{
    ...
    if (nvram_on_mtd)
        ...
        loaded = write(fd, destImage, imageSize);
    }
...
int enc_read(int fd, void *buffer, int rsize)
{
    ...
    if (nvram_on_mtd)
        ...
        loaded = read(fd, temp, ACTUAL_NVRAM_SIZE + sizeof(QBImageHeader));
...
int main(int argc, char *argv[])
{
    ...
    int ret, i, fd;
    char prim[0x10] = {0}, sec[0x10] = {0}, *p, *s;
    char buf[1024], fpath[1024];
    bool nvram_found = false, nvramB_found = false;
    ...
    if (!p || p == buf || !s || s == buf) {
        for (i = 1; i < 128; i++) {
            sprintf(fpath, "/sys/class/block/mmcblk0p%d/volname", i);
            if ((fd = open(fpath, O_RDONLY)) < 0 || read(fd, buf, sizeof(buf)) <= 0) {
                fprintf(stderr, "error: cant read %s file (fd = %d)\n", fpath, fd);
                cleanup();
                return errno;
            }
            close(fd);
            if (!strncmp("nvramB", buf, strlen("nvramB"))) {
                sprintf(sec, "/dev/mmcblk0p%d", i);
                nvramB_found = true;
            } else if (!strncmp("nvram", buf, strlen("nvram"))) {
                sprintf(prim, "/dev/mmcblk0p%d", i);
                nvram_found = true;
            }
            if (nvramB_found && nvram_found) {
                nvram_on_mtd = false;
                break;
            }
        }
