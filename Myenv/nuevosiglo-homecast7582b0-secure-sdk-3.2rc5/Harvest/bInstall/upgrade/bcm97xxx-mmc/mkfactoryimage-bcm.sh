...
if [ -z "$QBIMAGE_SPLASH" ]; then
	SPLASH_SIG=`jclient sign S_INSTALL_KEYSERVER_RSA_BOOT_HASH -t $SPLASH_IMG with S_INSTALL_KEYSERVER_RSA_BOOT_KEY_ID` || exit $?
...
if [ ! -z "$QBIMAGE_NVRAM" ]; then
	../host/local/usr/bin/mkqbbinaryimage.py --mode dynamic --encryption aes256-cbc --append-data nvram S_INSTALL_KEYSERVER_SYM_NVRAM_KEY_ID S_INSTALL_KEYSERVER_SYM_NVRAM_HMAC_KEY_ID $NVRAM_IMG ./nvram_a.qb
	../host/local/usr/bin/mkqbbinaryimage.py --mode dynamic --encryption aes256-cbc --append-data nvramB S_INSTALL_KEYSERVER_SYM_NVRAM_KEY_ID S_INSTALL_KEYSERVER_SYM_NVRAM_HMAC_KEY_ID $NVRAMB_IMG ./nvram_b.qb
