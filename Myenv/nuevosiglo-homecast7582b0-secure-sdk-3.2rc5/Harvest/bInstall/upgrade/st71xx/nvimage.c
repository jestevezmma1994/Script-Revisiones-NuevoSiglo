...
static char *envname = "nvram.bin";

#define CFG_ENV_SIZE  0x20000
#define ENV_SIZE      (CFG_ENV_SIZE - sizeof (uint32_t) - sizeof (char))
...
int fw_setenv (int argc, char *argv[])
{
	...
	*++env = '\0';
	...
	} // end of parameters processing loop
...
static int flash_io (int mode)
{
	...
	if ((fd = open (envname, mode == O_RDWR ? (O_RDWR | O_CREAT | O_TRUNC) : O_RDONLY, 00644)) < 0) {
		fprintf (stderr,
			"Can't open %s: %s\n",
...
static int env_init (void)
{
	if ((environment.data = calloc (1, ENV_SIZE)) == NULL) {
		fprintf (stderr,
	...
	if (flash_io (O_RDONLY) || (crc32 (0, environment.data, ENV_SIZE) != environment.crc)) {
		fprintf (stderr,
